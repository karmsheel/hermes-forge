    ...
    <div className={`card p-4 space-y-4 ${className}`}>
          ...
          {isConnected && config && (
            <div className="mt-1 text-[11px] text-zinc-500 font-mono truncate">
              ...
              {status.latencyMs != null && (
                <span className="text-zinc-600"> · {status.latencyMs}ms</span>
              )}
            </div>
          )}
      ...
      {!isConnected && status.error && (
        <div className="text-xs text-amber-300/90 bg-amber-500/10 border border-amber-500/20 rounded-lg px-3 py-2">
          {status.error}
          {tip && <div className="mt-1.5 text-zinc-400">{tip}</div>}
        </div>
      )}
      ...
      <div>
        ...
        {manualOpen && (
          <div className="mt-3 space-y-3">
            <div>
              <label className="text-[10px] uppercase tracking-widest text-zinc-500">API URL</label>
              <input
