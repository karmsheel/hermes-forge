...
export async function POST(
  ...
  try {
    ...
    if (agent.isHired) {
      return NextResponse.json({ error: 'Agent is already hired' }, { status: 409 });
    }

    const hiredAt = new Date();
    const updated = await prisma.hermesAgentProfile.update({
      where: { id: agent.id },
      data: { isHired: true, hiredAt },
    });

    await recordBusinessEvent({
      businessId: business.id,
      userId: session.userId,
      type: BUSINESS_EVENT_TYPES.PERSONNEL_HIRED,
      entityType: 'personnel',
      entityId: updated.id,
      entityName: updated.displayName,
      summary: `Hired agent "${updated.displayName}" for ${business.name}`,
      metadata: {
        kind: 'agent',
