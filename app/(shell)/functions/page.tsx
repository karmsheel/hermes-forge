...
export default function FunctionsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setBusinessId(data.business?.id || currentBusiness?.id || null);
      setBizName(data.business?.name || currentBusiness?.name || null);
      setFunctions(aggregateFunctions(procs));
    } catch {
  ...
  }, [router, currentBusiness?.id, currentBusiness?.name]);
          ...
          {functions.map((fn) => (
            <li key={fn.name}>
              <div
                role="button"
                tabIndex={0}
                onClick={() => openFunctionInWorkshop(fn.name)}
                onKeyDown={(e) => {
                  if (e.key === "Enter" || e.key === " ") openFunctionInWorkshop(fn.name);
                }}
